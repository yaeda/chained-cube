...
export class Cube {
  ...
  private static CHARACTERISTIC_ID_UUID = "10B20101-5B3B-4571-9508-CF3EFCD7BBAE".toLowerCase();
  ...
  constructor(device: BluetoothDevice) {
    this.device = device;
    this.id = this.device.gatt?.device.id ?? "undefined";
  ...
  public async connect() {
    ...
    // id monitoring
    const idChar = await this.characteristics
      .get(Cube.CHARACTERISTIC_ID_UUID)
      ?.startNotifications();
    idChar?.addEventListener(
      "characteristicvaluechanged",
      this.handleIdNotification
    );
  ...
  private handleIdNotification = (event: Event) => {
      ...
      case 0x01:
        {
          ...
          this.eventEmitter.emit("id:position-id", { x, y, angle });
        }
        break;
      case 0x02:
        {
          const standardId = data.getUint32(1, true);
          const angle = data.getUint16(5, true);
          this.eventEmitter.emit("id:standard-id", { standardId, angle });
        }
        break;
      case 0x03:
        this.eventEmitter.emit("id:position-id-missed");
        break;
      case 0x04:
        this.eventEmitter.emit("id:standard-id-missed");
        break;
      default:
        break;
    }
